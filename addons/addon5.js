// addons/addon5.js - Mod Toggle Menu
// Adds a fully integrated Mod Manager to the main menu to enable/disable specific addons.
...
setTimeout(() => {
    ...
    // Intercept standard button inputs
    if (typeof handleUIEvent === 'function') {
        const originalHandleUI = handleUIEvent;
        handleUIEvent = function(inputId, actionType) {
            
            // Open Mods from Main Menu using Y button mapping (ccToggle)
            if (gameState.phase === 'menu' && actionType === 'ccToggle' && (inputId === 0 || inputId === 2)) {
                changePhase('mods');
                return;
            }
            
            // Logic for Mod Manager Screen
            if (gameState.phase === 'mods' && (inputId === 0 || inputId === 2)) {
                if (actionType === 'up') {
                    ...
                    // Save to local storage and force a hard refresh to re-evaluate injected scripts
